...
def generate_code_relocation(dep):
    defs_file = env.File(os.path.join("$BUILD_DIR", ".code_relocation_defs"))
    code_relocation_defs = defs_file.get_contents().decode("utf8")
    # not sure why I need to do this :(
    code_relocation_defs = code_relocation_defs.replace(".c", ".c;").strip(';')
    cmd = [
        "$PYTHONEXE",
        '"%s"' % os.path.join(FRAMEWORK_DIR, "scripts", "gen_relocate_app.py"),
        "-d","$BUILD_DIR",
        "-i", f'"{code_relocation_defs}"',
        "-o", "$BUILD_DIR/zephyr/include/generated/linker_relocate.ld",
        "-s", "$BUILD_DIR/zephyr/include/generated/linker_sram_data_relocate.ld",
        "-b", "$BUILD_DIR/zephyr/include/generated/linker_sram_bss_relocate.ld",
        "-c", "$BUILD_DIR/zephyr/code_relocation.c",
    ]

    return env.Command(
        os.path.join("$BUILD_DIR", "zephyr", "code_relocation.c"),
        dep,
        env.VerboseAction(" ".join(cmd), "Generating code relocations $TARGET")
    )
...
for target, target_config in target_configs.items():
    ...
    if (
        ...
        or lib_name in ("app", "offsets", "code_relocation_source_lib")
    ):
        continue
...
def find_framework_module(name):
    for l in framework_modules_map.values():
        if l['config']['name'] == name:
            return l['lib_path']
    return None

code_relocation_source_lib = build_library(env, target_configs["code_relocation_source_lib"], PROJECT_SRC_DIR)
env.Depends(code_relocation_source_lib[0].sources, offset_header_file)

if project_settings.get("CONFIG_CODE_DATA_RELOCATION", ""):
    # these dependencies
    kernel_lib = find_framework_module("kernel")
    code_reloc = generate_code_relocation(kernel_lib)
    env.Depends(base_ld_script, code_reloc)
...
for dep in (isr_table_file, final_ld_script, code_relocation_source_lib):
